...
const showTags = () => {
    ...
    categories.forEach(item => {
        ...
        const param = JSON.stringify([item.item]);
        console.log(param);
        div.innerHTML = `
                        <div class="cat-item d-flex flex-column border mb-4" style="padding: 30px;">
                            <a href='shop.html?marcas=${param}' class="cat-img position-relative overflow-hidden mb-3">
                                <img class="img-fluid" src="${item.imgUrl}">
                            </a>
...
const showProductos = () => {
    ...
    const tendencia = listaProductos.slice(0,8);
    tendencia.forEach(producto => {
        ...
        perfumeCard.innerHTML = `
                                <div class="card product-item border-0 mb-4">
                                    ...
                                    <div class="card-footer d-flex justify-content-between bg-light border">
                                        <a href="shop.html" class="btn btn-sm text-dark p-0"><i class="fas fa-shopping-cart text-primary mr-1"></i>Ir a la tienda</a>
                                    </div>
                                </div>`
